/**
 * 常用地理坐标系转换工具类
 * 提供了百度坐标系(BD-09)、火星坐标系(GCJ-02，又称国测局坐标系)、地球坐标系(WGS84，又称经纬度)之间的互相转换方法
 * 提供了百度经纬度与百度墨卡托之间的互相转换方法
 * 本工具类适合用于将适量数量点坐标加减偏移用于叠加不同网络底图
 * 本工具类不适合成千上万等大量点做偏移量修正
 */


/**
 * 关于解决百度地图、顺丰地图、arcgis地图坐标相互转换出现的偏移问题：
 * （1）百度地图转顺丰地图，使用bd09ToWgs84()转成wgs84坐标；
 * （2）百度地图转arcgis地图，使用bd09ToGcj02()转成gcj02坐标；
 * （3）arcgis地图转顺丰地图，使用wgs84ToGcj02()转成gcj02坐标；
 * （4）arcgis地图转百度地图，使用gcj02ToBd09()转成bd09坐标；
 * （5）顺丰地图转百度地图，使用wgs84ToBd09()转成bd09坐标；
 * （6）顺丰地图转arcgis地图，使用gcj02ToWgs84()转成wgs84坐标；
 */


// 常量
const PI = 3.1415926535897932384626;  // π
const x_PI = 3.14159265358979324 * 3000.0 / 180.0;
const a = 6378245.0;  // 长半轴
const ee = 0.00669342162296594323;  // 偏心率平方
const EARTHRADIUS = 6370996.81;
const MCBAND = [12890594.86, 8362377.87, 5591021, 3481989.83, 1678043.12, 0];
const LLBAND = [75, 60, 45, 30, 15, 0];
const MC2LL = [
  [1.410526172116255e-8, 0.00000898305509648872, -1.9939833816331, 200.9824383106796, -187.2403703815547, 91.6087516669843, -23.38765649603339, 2.57121317296198, -0.03801003308653, 17337981.2],
  [-7.435856389565537e-9, 0.000008983055097726239, -0.78625201886289, 96.32687599759846, -1.85204757529826, -59.36935905485877, 47.40033549296737, -16.50741931063887, 2.28786674699375, 10260144.86],
  [-3.030883460898826e-8, 0.00000898305509983578, 0.30071316287616, 59.74293618442277, 7.357984074871, -25.38371002664745, 13.45380521110908, -3.29883767235584, 0.32710905363475, 6856817.37],
  [-1.981981304930552e-8, 0.000008983055099779535, 0.03278182852591, 40.31678527705744, 0.65659298677277, -4.44255534477492, 0.85341911805263, 0.12923347998204, -0.04625736007561, 4482777.06],
  [3.09191371068437e-9, 0.000008983055096812155, 0.00006995724062, 23.10934304144901, -0.00023663490511, -0.6321817810242, -0.00663494467273, 0.03430082397953, -0.00466043876332, 2555164.4],
  [2.890871144776878e-9, 0.000008983055095805407, -3.068298e-8, 7.47137025468032, -0.00000353937994, -0.02145144861037, -0.00001234426596, 0.00010322952773, -0.00000323890364, 826088.5]
];
const LL2MC = [
  [-0.0015702102444, 111320.7020616939, 1704480524535203, -10338987376042340, 26112667856603880, -35149669176653700, 26595700718403920, -10725012454188240, 1800819912950474, 82.5],
  [0.0008277824516172526, 111320.7020463578, 647795574.6671607, -4082003173.641316, 10774905663.51142, -15171875531.51559, 12053065338.62167, -5124939663.577472, 913311935.9512032, 67.5],
  [0.00337398766765, 111320.7020202162, 4481351.045890365, -23393751.19931662, 79682215.47186455, -115964993.2797253, 97236711.15602145, -43661946.33752821, 8477230.501135234, 52.5],
  [0.00220636496208, 111320.7020209128, 51751.86112841131, 3796837.749470245, 992013.7397791013, -1221952.21711287, 1340652.697009075, -620943.6990984312, 144416.9293806241, 37.5],
  [-0.0003441963504368392, 111320.7020576856, 278.2353980772752, 2485758.690035394, 6070.750963243378, 54821.18345352118, 9540.606633304236, -2710.55326746645, 1405.483844121726, 22.5],
  [-0.0003218135878613132, 111320.7020701615, 0.00369383431289, 823725.6402795718, 0.46104986909093, 2351.343141331292, 1.58060784298199, 8.77738589078284, 0.37238884252424, 7.45]
];

/**
 * 百度坐标系(BD-09)转火星坐标系(GCJ-02)/即百度转谷歌、高德
 * @param { Number } bd_lon
 * @param { Number } bd_lat
 */
const bd09ToGcj02 = (bd_lon, bd_lat) => {
  let x = bd_lon - 0.0065;
  let y = bd_lat - 0.006;
  let z = Math.sqrt(x * x + y * y) - 0.00002 * Math.sin(y * x_PI);
  let theta = Math.atan2(y, x) - 0.000003 * Math.cos(x * x_PI);
  let gg_lon = z * Math.cos(theta);
  let gg_lat = z * Math.sin(theta);
  return [gg_lon, gg_lat];
};

/**
 * 火星坐标系(GCJ-02)转百度坐标系(BD-09)/即谷歌、高德转百度
 * @param { Number } lon
 * @param { Number } lat
 */
const gcj02ToBd09 = (lon, lat) => {
  let z = Math.sqrt(lon * lon + lat * lat) + 0.00002 * Math.sin(lat * x_PI);
  let theta = Math.atan2(lat, lon) + 0.000003 * Math.cos(lon * x_PI);
  let bd_lon = z * Math.cos(theta) + 0.0065;
  let bd_lat = z * Math.sin(theta) + 0.006;
  return [bd_lon, bd_lat];
};

/**
 * 地球坐标系(WGS84)转火星坐标系(GCJ-02)/即谷歌地球转谷歌、高德
 * @param { Number } lon
 * @param { Number } lat
 */
const wgs84ToGcj02 = (lon, lat) => {
  if (_outOfChina(lon, lat)) {
    return [lon, lat];
  } else {
    let dlat = _transformLat(lon - 105.0, lat - 35.0);
    let dlon = _transformLng(lon - 105.0, lat - 35.0);
    let radlat = lat / 180.0 * PI;
    let magic = Math.sin(radlat);
    magic = 1 - ee * magic * magic;
    let sqrtmagic = Math.sqrt(magic);
    dlat = (dlat * 180.0) / ((a * (1 - ee)) / (magic * sqrtmagic) * PI);
    dlon = (dlon * 180.0) / (a / sqrtmagic * Math.cos(radlat) * PI);
    const mglat = lat + dlat;
    const mglon = lon + dlon;
    return [mglon, mglat];
  }
};

/**
 * 火星坐标系(GCJ-02)转地球坐标系(WGS84)/即谷歌、高德转谷歌地球
 * @param { Number } lon
 * @param { Number } lat
 */
const gcj02ToWgs84 = (lon, lat) => {
  if (_outOfChina(lon, lat)) {
    return [lon, lat];
  } else {
    let dlat = _transformLat(lon - 105.0, lat - 35.0);
    let dlon = _transformLng(lon - 105.0, lat - 35.0);
    let radlat = lat / 180.0 * PI;
    let magic = Math.sin(radlat);
    magic = 1 - ee * magic * magic;
    let sqrtmagic = Math.sqrt(magic);
    dlat = (dlat * 180.0) / ((a * (1 - ee)) / (magic * sqrtmagic) * PI);
    dlon = (dlon * 180.0) / (a / sqrtmagic * Math.cos(radlat) * PI);
    const mglat = lat + dlat;
    const mglon = lon + dlon;
    return [lon * 2 - mglon, lat * 2 - mglat];
  }
};

/**
 * 百度坐标系(BD-09)转地球坐标系(WGS84)/即百度转谷歌地球
 * @param { Number } lon
 * @param { Number } lat
 */
const bd09ToWgs84 = (lon, lat) => {
  // 百度坐标系先转为火星坐标系
  const gcj02 = bd09ToGcj02(lon, lat);
  // 火星坐标系转地球坐标系
  return gcj02ToWgs84(gcj02[0], gcj02[1]);
};

/**
 * 地球坐标系(WGS84)转百度坐标系(BD-09)/即谷歌地球转百度
 * @param { Number } lon
 * @param { Number } lat
 */
const wgs84ToBd09 = (lon, lat) => {
  // 地球坐标系先转为火星坐标系
  const gcj02 = wgs84ToGcj02(lon, lat);
  // 火星坐标系转百度坐标系
  return gcj02ToBd09(gcj02[0], gcj02[1]);
};

/**
 * 百度经纬度(BD-09)转百度墨卡托
 * @param { Number } lng
 * @param { Number } lat
 */
const bd09ToBdMkt = (lng, lat) => {
  let cF = null;
  lng = _getLoop(lng, -180, 180);
  lat = _getRange(lat, -74, 74);
  for (let i = 0; i < LLBAND.length; i++) {
    if (lat >= LLBAND[i]) {
      cF = LL2MC[i];
      break;
    }
  }
  if (cF!=null) {
    for (let i = LLBAND.length - 1; i >= 0; i--) {
      if (lat <= -LLBAND[i]) {
        cF = LL2MC[i];
        break;
      }
    }
  }
  lng = cF[0] + cF[1] * Math.abs(lng);
  let cC = Math.abs(lat) / cF[9];
  lat = cF[2] + cF[3] * cC + cF[4] * cC * cC + cF[5] * cC * cC * cC + cF[6] * cC * cC * cC * cC + cF[7] * cC * cC * cC * cC * cC + cF[8] * cC * cC * cC * cC * cC * cC;
  lng *= (lng < 0 ? -1 : 1);
  lat *= (lat < 0 ? -1 : 1);
  return [lng, lat];
};

/**
 * 百度墨卡托转百度经纬度(BD-09)
 * @param { Number } lng
 * @param { Number } lat
 */
const bdMktToBd09 = (lng, lat) => {
  let cF = null;
  lng = Math.abs(lng);
  lat = Math.abs(lat);
  for(let cE = 0; cE < MCBAND.length; cE++) {
    if (lat >= MCBAND[cE]) {
      cF = MC2LL[cE];
      break;
    }
  }
  lng = cF[0] + cF[1] * Math.abs(lng);
  let cC = Math.abs(lat) / cF[9];
  lat = cF[2] + cF[3] * cC + cF[4] * cC * cC + cF[5] * cC * cC * cC + cF[6] * cC * cC * cC * cC + cF[7] * cC * cC * cC * cC * cC + cF[8] * cC * cC * cC * cC * cC * cC;
  lng *= (lng < 0 ? -1 : 1);
  lat *= (lat < 0 ? -1 : 1);
  return [lng, lat];
};

/**
 * 地球经纬度(WGS84)转Web墨卡托
 * @param { Number } lng
 * @param { Number } lat
 */
const wgs84ToWebMkt = (lng, lat) => {
  lng = lng * 20037508.34 / 180;
  lat = Math.log(Math.tan((90 + lat) * PI / 360)) / (PI / 180);
  lat = lat * 20037508.34 / 180;
  return [lng, lat];
};

/**
 * Web墨卡托转地球经纬度(WGS84)
 * @param { Number } lng
 * @param { Number } lat
 */
const webMktToWgs84 = (lng, lat) => {
  lng = lng / 20037508.34 * 180;
  lat = lat / 20037508.34 * 180;
  lat = 180 / PI * (2 * Math.atan(Math.exp(lat * PI / 180)) - PI / 2);
  return [lng, lat];
};

const _transformLat = (lng, lat) => {
  let ret = -100.0 + 2.0 * lng + 3.0 * lat + 0.2 * lat * lat + 0.1 * lng * lat + 0.2 * Math.sqrt(Math.abs(lng));
  ret += (20.0 * Math.sin(6.0 * lng * PI) + 20.0 * Math.sin(2.0 * lng * PI)) * 2.0 / 3.0;
  ret += (20.0 * Math.sin(lat * PI) + 40.0 * Math.sin(lat / 3.0 * PI)) * 2.0 / 3.0;
  ret += (160.0 * Math.sin(lat / 12.0 * PI) + 320 * Math.sin(lat * PI / 30.0)) * 2.0 / 3.0;
  return ret;
};

const _transformLng = (lng, lat) => {
  let ret = 300.0 + lng + 2.0 * lat + 0.1 * lng * lng + 0.1 * lng * lat + 0.1 * Math.sqrt(Math.abs(lng));
  ret += (20.0 * Math.sin(6.0 * lng * PI) + 20.0 * Math.sin(2.0 * lng * PI)) * 2.0 / 3.0;
  ret += (20.0 * Math.sin(lng * PI) + 40.0 * Math.sin(lng / 3.0 * PI)) * 2.0 / 3.0;
  ret += (150.0 * Math.sin(lng / 12.0 * PI) + 300.0 * Math.sin(lng / 30.0 * PI)) * 2.0 / 3.0;
  return ret;
};

/**
 * 判断是否在国内，不在国内则不做偏移
 * @param { Number } lng
 * @param { Number } lat
 */
const _outOfChina = (lng, lat) => {
  return (lng < 72.004 || lng > 137.8347) || ((lat < 0.8293 || lat > 55.8271) || false);
};

const _getLoop = (lng, min, max) => {
  while (lng > max) {
    lng -= max - min;
  }
  while (lng < min) {
    lng += max - min;
  }
  return lng;
};

const _getRange = (lat, min, max) => {
  if (min != null) {
    lat = Math.max(lat, min);
  }
  if (max != null) {
    lat = Math.min(lat, max);
  }
  return lat;
};

export default {
  // 百度坐标系(BD-09)转火星坐标系(GCJ-02)/即百度转谷歌、高德
  bd09ToGcj02,
  // 火星坐标系(GCJ-02)转百度坐标系(BD-09)/即谷歌、高德转百度
  gcj02ToBd09,
  // 地球坐标系(WGS84)转火星坐标系(GCJ-02)/即谷歌地球转谷歌、高德
  wgs84ToGcj02,
  // 火星坐标系(GCJ-02)转地球坐标系(WGS84)/即谷歌、高德转谷歌地球
  gcj02ToWgs84,
  // 百度坐标系(BD-09)转地球坐标系(WGS84)/即百度地转谷歌地球
  bd09ToWgs84,
  // 地球坐标系(WGS84)转百度坐标系(BD-09)/即谷歌地球转百度
  wgs84ToBd09,
  // 百度经纬度(BD-09)转百度墨卡托
  bd09ToBdMkt,
  // 百度墨卡托转百度经纬度(BD-09)
  bdMktToBd09,
  // 地球经纬度(WGS84)转Web墨卡托
  wgs84ToWebMkt,
  // Web墨卡托转地球经纬度(WGS84)
  webMktToWgs84
};
