<template>
  <el-table-column
    :fixed="coloumnHeader.prop === 'orgName' || coloumnHeader.prop === 'zl'"
    ...
    <template slot-scope="scope" v-if="!coloumnHeader.children || coloumnHeader.children.length === 0">
      <div class="column-org-name" v-if="coloumnHeader.prop === 'orgName'">
        <a href="javascript:;" class="canClick" @click="handleList(coloumnHeader.prop, scope.row)" v-if="!scope.row.isLeaf" style="cursor: pointer;text-decoration: underline">
         {{ '&nbsp;' + scope.row[coloumnHeader.prop] + '&nbsp;' }}
        </a>
        <span v-else class="unClick">
          {{scope.row[coloumnHeader.prop]}}
        </span>
      </div>
      <div class="column-num" :class="[scope.row[coloumnHeader.prop] === 0 ? 'unClick' : 'canClick']" v-else>
