<template>
  <div class="hdty-container hdty-flex" style="padding: 0">
          ...
          <hd-expand>
            ...
            <hd-expand-body>
              ...
              <el-row>
                ...
                <el-form-item label="注销状态" prop="zxbs" v-if="!isSp">
                  <hd-dict-radio
        ...
        <div class="hdty-task" v-for="(item, index) in tableData" :key="index">
          <div class="hdty-task-body" @click="handleDetail(index, item)">
            <div class="hdty-task-row">
              ...
              <div style="margin-left: 12px" v-if="item.vo.spzt === '2'">
                <el-tag
                  size="mini"
                  type="info"
                  effect="dark"
            ...
            <div class="hdty-task-row">
              ...
              <div class="hdty-task-row-item row-item-12">
                <div class="con-flex con-i-label-info">
                  <i class="el-icon-time"></i>
                  <div>创建时间：</div>
                  <div>{{ item.vox.createTime || "暂无创建时间" }}</div>
                </div>
              </div>
            </div>
            <div class="hdty-task-row">
              ...
              <div class="hdty-task-row-item row-item-12">
                <div class="con-flex con-i-label-info">
                  ...
                  <div>创建人：</div>
                  <div>{{ item.vo.createUsername || "暂无创建人" }}</div>
                </div>
              </div>
              <div class="hdty-task-row-item row-item-12">
                <div class="con-flex con-i-label-info">
                  ...
                  <div>所属部门：</div>
                  <div>{{ item.vox.jzSsbm || "暂无所属部门" }}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="hdty-task-operate">
            <div>
              <!-- 我创建的显示 -->
              <hd-button
                v-if="!isSp && item.vo.spzt === '0' && item.vo.zxbs != '1'"
                name="edit"
                size="mini"
                type="primary"
              ...
              <!-- 审批显示 -->
              <hd-button
                v-if="isSp && item.vo.spzt === '1' && item.vo.zxbs != '1'"
                name="edit"
                size="mini"
                type="primary"
                @click="handleExamine(index, item)"
                >审批</hd-button
              >
              ...
              >撤销</hd-button>
              <hd-button
                v-if="false && item.vo.zxbs === '0' && !['1','4'].includes(item.vo.taskState)"
                size="mini"
                type="warning"
    ...
    <new-task-center-dis-detail-examine
      :isSp="isShowForm"
      ref="detailRef"
    ></new-task-center-dis-detail-examine>
    ...
    ></new-task-center-dis-modify>
    <!-- 审批 -->
    <!-- <new-task-center-dis-examine ref="NewTaskCenterDisExamineRef"></new-task-center-dis-examine> -->
  </div>
</template>
<script>
import { hdList } from "@/utils/util-framework";
import { getTaskPageSp, submitTaskSp, restoreTask } from "./../api";
import NewTaskCenterDisAdd from "./NewTaskCenterDisAdd";
import NewTaskCenterDisDetailExamine from "./NewTaskCenterDisDetailExamine";
...
import NewTaskCenterDisExamine from "./NewTaskCenterDisExamine.vue";
...
export default {
  ...
  components: {
    NewTaskCenterDisAdd,
    NewTaskCenterDisDetailExamine,
    NewTaskCenterDisLogout,
    NewTaskCenterDisTaskRevoke,
    NewTaskCenterDisModify,
    NewTaskCenterDisExamine
  },
  data() {
    return {
      // 所属机构可查询到市级
      filterOrg: this.$utilPublic.getUserInfo().orgId.substr(0, 4) + "00000000",
      // 查询
      queryData: {
        yqType: "",
        isPc: "1",
