              ...
              <hd-component :model="factorData" ref="componentRef">
                <el-form-item label="表单名称" prop="formTitle">
                  <hd-factor-input
                    ...
                    :model-contorl.sync="factorData.formTitleFilterFactor"
                    fuzzy="between"
                    placeholder="请输入表单名称"
                  ></hd-factor-input>
                </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      handleChecked(index, row) {
        this.visible = false;
        this.input = row.vo.formTitle;
        this.$emit('input', row.vo.formId); //触发 input 事件，并传入新值
        this.$emit('update:modelText', row.vo.formTitle);
      ...
      toggleSelection() {
        ...
        if (_this.$refs.tableRef) {
          ...
          _this.multipleData.forEach(item => {
            let currentSelections = _this.tableData.find(e => e.vo.formId === item.vo.formId);
          ...
          _this.tableData.forEach(item => {
            surplusSelections = surplusSelections.filter(e => e.vo.formId !== item.vo.formId);
          });
      ...
      handleSelect(val) {
        ...
        if (_this.triggerSelectionValidate(val)) {
          ...
          _this.tableData.forEach(item => {
            selections = selections.filter(e => e.vo.formId !== item.vo.formId);
          });
          val.forEach(item => {
            selections = selections.filter(e => e.vo.formId !== item.vo.formId);
          });
      ...
      submitForm() {
        ...
        this.$emit('input', this.$utilStr.objArrayToArray(this.$utilStr.objArrayToArray(this.multipleData, 'vo'), 'formId')); //触发 input 事件，并传入新值
        this.$emit('update:modelText', this.$utilStr.objArrayToArray(this.$utilStr.objArrayToArray(this.multipleData, 'vo'), 'formTitle'));
      ...
      // input改变
      triggerInputChange() {
        this.input = this.$utilStr.splitArray(this.$utilStr.objArrayToArray(this.$utilStr.objArrayToArray(this.multipleData, 'vo'), 'formTitle'));
      },
      // 取消
      cancelForm() {
        this.visible = false;
