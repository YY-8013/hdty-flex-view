    ...
    <div class="con-file-all">
      <div class="con-file-name">
        ...
        >
          ...
          <div class="con-name hdty-elip" :title="fileItem.name">
            {{ fileItem.name }}
          </div>
          <div class="con-btn" :disabled="disabled" v-show="deleteFileSubItem">
            <i
              class="el-icon-delete"
              title="点击删除附件"
              @click.stop="handleDelete(fileItem)"
            ></i>
          </div>
        </div>
        <div v-if="!fileList || fileList.length === 0" class="con-empty">
          暂无附件
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    uploadButtonText() {
      if (this.limit && Number(this.limit) === 1 && this.fileList.length > 0) {
        return "重新上传";
      }
      return "点击上传";
    },
  ...
  methods: {
    ...
    handleUploadFile(file) {
      ...
      fileFormData.append(_this.fileProp, file.raw);
      let dataParams = {
        params: {
      ...
      _this
        ...
        .then((response) => {
          ...
          if (success) {
            ...
            let fileObj = {
              name: file.name,
              url: data[_this.uploadProp.url],
              id: data[_this.uploadProp.id]
            };
