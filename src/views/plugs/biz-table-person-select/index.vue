  ...
  >
    ...
    >
      <div slot="title" class="hdty-dialog-header">
        ...
        <div v-if="refresh" class="hdty-dialog-refresh" title="点击刷新" @click="reload(true)"></div>
      </div>
      <hd-pane>
      ...
      <div slot="footer" class="hdty-dialog-footer">
        <div class="hdty-float-left">
          ...
          >{{ $utilStr.getValueOfDeepObj(activeTag[0], defaultProps['label']) }}</el-tag>
          <el-tag
        ...
        <div class="hdty-btn-container">
          ...
          <el-button v-if="multiple" type="primary" @click="submit">确 定</el-button>
        </div>
      </div>
    ...
    setSelectionSearchData() {
      if (
        !this.$utilStr.isEqual(
          this.queryData[this.defaultProps['selection']],
          this.currentValue
        )
      ) {
        let selection = this.currentValue;
        if (
          Array.isArray(this.queryData[this.defaultProps['selection']]) &&
    ...
    loadList() {
      ...
      page(dataParams)
        .then((response) => {
          ...
          if (success) {
            if (this.$utilStr.isNotEmptyObject(this.pageOptions)) {
              this.pageOptions.total = this.$utilStr.getValueOfDeepObj(
                data,
                this.defaultProps['total']
              );
              this.cacheData.pageOptions = this.$utilStr.deepClone(
                this.pageOptions
              );
            }
      ...
      if (this.$refs.tableRef) {
        ...
        if (this.tableData && this.tableData.length > 0) {
          currentRow = this.tableData.find(
            (e) =>
              this.$utilStr.getValueOfDeepObj(e, this.defaultProps['value']) ===
              this.currentValue
          );
        }
      ...
      if (this.validateLimit(val)) {
        ...
        val.forEach((item) => {
          selections = selections.filter(
            (e) =>
              this.$utilStr.getValueOfDeepObj(e, this.defaultProps['value']) !==
              this.$utilStr.getValueOfDeepObj(item, this.defaultProps['value'])
          );
        });
