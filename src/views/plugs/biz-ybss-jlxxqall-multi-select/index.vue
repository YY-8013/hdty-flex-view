<template>
  <div>
    ...
    ></el-input>
    <el-dialog :title="title" :visible.sync="visible" custom-class="hdty-dialog-medium" append-to-body>
      <hd-pane>
          ...
          <div class="hdty-header">
            <el-form
              ...
              <hd-component :model="factorData" ref="componentRef">
                <el-form-item label="街路巷(小区/村庄)名称" prop="jlxxqmcQc">
                  <hd-factor-input
                    prop="jlxxqmcQcFilterFactor"
                    v-model="queryData.jlxxqmcQc"
                    fuzzy="between"
                    modelControl="likeRight"
                    :model-control.sync="factorData.jlxxqmcQcFilterFactor"
                    placeholder="请输入街路巷(小区/村庄)名称"
                  ></hd-factor-input>
                </el-form-item>
                <el-form-item label="街路巷(小区)类型" prop="dzyslxdm" v-if="showJlxlx">
                  <hd-dict-checkbox
                    ...
                    multiple :currentValue="queryData.dzyslxdm"
                    :remove-option="['21','22','23','31','32','33','34', '50', '60', '70','41','43','80']"
                    showMultiButton
                    filterable
                  ></hd-dict-checkbox>
                </el-form-item>
      ...
      <span slot="footer" class="hdty-dialog-footer">
        <cite class="hdty-float-left">当前选择：
          <el-tag v-if="input" closable type="success" @close="handleTagClose">{{ input }}</el-tag>
        </cite>
...
<script>
  ...
  export default {
    ...
    provide() {
      return {
        BizYbssJlxxqallMultiSelect: this
      };
    },
    ...
    data() {
      return {
        ...
        // 查询条件
        factorData: {
          jlxxqmcQcFilterFactor: this.$app.factorOptions.matchOpe.between
        },
        // 验证
        rules: {},
        // 表格数据
        ...
        // 启用构造
        activeEnable: this.multiple && this.value.length > 0 && this.currentSelection.length < 1,
        // 构造数据
        activeData: [],
        // 页面加载次数
        loadTime: 0,
    ...
    methods: {
      ...
      loadList() {
        let _this = this;
        if(_this.loadTime < 1 || !_this.queryData.dzyslxdm || _this.queryData.dzyslxdm.length < 1) {
        ...
        listPage(dataParams).then(response => {
          // 响应成功回调
          let {
            success,
            data
      ...
      toggleSelection() {
        ...
        if (_this.$refs.tableRef) {
          ...
          _this.multipleData.forEach(item => {
            let currentSelections = _this.tableData.find(e => e.vo.dzbm === item.vo.dzbm);
          ...
          _this.tableData.forEach(item => {
            surplusSelections = surplusSelections.filter(e => e.vo.dzbm !== item.vo.dzbm);
          });
      ...
      handleSelect(val) {
        ...
        if (_this.triggerSelectionValidate(val)) {
          ...
          _this.tableData.forEach(item => {
            selections = selections.filter(e => e.vo.dzbm !== item.vo.dzbm);
          });
          val.forEach(item => {
            selections = selections.filter(e => e.vo.dzbm !== item.vo.dzbm);
          });
        ...
        if (_this.multipleLimit > 0) {
          if (val.length > _this.multipleLimit) {
            _this.$message({
              ...
              message: '超出多选限制，最多选择[' + _this.multipleLimit + ']项数据',
              type: 'error'
            });
      ...
      submitForm() {
        ...
        this.$emit('input', this.$utilStr.objArrayToArray(this.$utilStr.objArrayToArray(this.multipleData, 'vo'), 'dzbm')); //触发 input 事件，并传入新值
        this.$emit('update:modelText', this.$utilStr.objArrayToArray(this.$utilStr.objArrayToArray(this.multipleData, 'vo'), 'jlxxqmcQc'));
      ...
      // input改变
      triggerInputChange() {
        this.input = this.$utilStr.splitArray(this.$utilStr.objArrayToArray(this.$utilStr.objArrayToArray(this.multipleData, 'vo'), 'jlxxqmcQc'));
      },
      // 取消
      cancelForm() {
        this.visible = false;
