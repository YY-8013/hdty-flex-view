...
<script>
  ...
  import { edit, editBefore } from '@/api/delrule';
  export default {
    name: 'DelRuleEdit',
    provide() {
      return {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          dataRange: [
            { required: true, message: '请选择删除范围', trigger: 'change' }
          ],
          remarks: [
      ...
      loadForm() {
        ...
        editBefore(dataParams).then(response => {
          ...
          if (success) {
            ...
            _this.handleDataSourceChange(data.vo.dataSource);
          }
        }).catch(error => {
          // 响应错误回调
          console.error(error);
