  ...
  export default {
      ...
      activeMenuKey() {
        ...
        (function fn(key, value, items, resultPath) {
          ...
          for (let index = 0; index < items.length; index++) {
            const e = items[index];
            checkResult = e[key] === value || (e.children && fn(key, value, e.children, resultPath));
            if (checkResult) {
      ...
      navigations: {
        handler(val) {
          if (this.activeMenuDefaultExpandFirst && !this.activeMenuKey) {
            if (this.activeMenuDefaultExpandFirst === '*' || this.navigations.length <= this.activeMenuDefaultExpandFirst) {
              if (val && val.length > 0) {
    ...
    methods: {
      handleSelect(key) {
        ...
        let routeMetaData = routeResolveData && routeResolveData.route && routeResolveData.route.meta;
        if (routeMetaData && routeMetaData.viewType === this.$global.dictItem.routerViewType.App) {
