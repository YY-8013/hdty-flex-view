<template>
  ...
  >
    ...
    <div
      v-else-if="activeScheme === 'upanddown' && !bodypage"
      class="hdty-header"
    >
      <topbar
        v-model="currentTopnavIndex"
    ...
    <div v-if="!bodypage" class="hdty-side hdty-side-menu">
      <div v-if="false && activeScheme === 'upanddown'" class="hdty-side-top">
        <hd-avatar :avatar="$utilLogin.getUserAvatar()"></hd-avatar>
        <div class="hdty-person-title">
          <p class="hdty-person-author hdty-elip">
            {{ user.xm ? user.xm : user.username }}
          </p>
        ...
        <div class="hdty-btn-container">
          <el-button
            :title="menucollapEnabled ? '个人中心' : ''"
            @click="handleMyCenter"
            >个人中心</el-button
          >
          <el-button
            :title="menucollapEnabled ? '修改密码' : ''"
            @click="handleChangePwd"
            >修改密码</el-button
          >
          <el-button
            :title="menucollapEnabled ? '退出登录' : ''"
            @click="handleLogout"
            >退出登录</el-button
          >
        </div>
      ...
      <div class="hdty-menu">
        ...
        <div class="hdty-menu-body hdty-dp-scroll">
          <side-menu
            :navigations="currentSubnavs"
            :collapse="menucollapEnabled"
          ></side-menu>
        </div>
      </div>
    ...
    <div v-if="pagetabEnabled && !bodypage" class="hdty-pagetabs">
      ...
      ></page-tabs>
      <collapse
        v-if="activeScheme === 'upanddown'"
        ref="collapseRef"
        :badge="badgeEnabled"
      ></collapse>
    </div>
    <!-- 主体区 -->
    <div class="hdty-body dp-workbench">
      ...
      <component
        ...
        :is="item.component"
        :class="
          $route.meta.keepAlive && item.path === $route.fullPath
            ? ''
            : 'hdty-hide'
        "
      ></component>
    ...
    resetLayout() {
      this.$nextTick(() => {
        ...
        for (let i = o.length - 1; i >= 0; i--) {
          if (!o[i].classList || !o[i].classList.contains("hdty-body")) {
            o[i].remove();
          }
        }
      });
