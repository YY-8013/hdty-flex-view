/**
 * 百度地图几何图形工具类
 * 提供若干几何算法，用来判断点与矩形、圆形、多边形线、多边形面的关系，并提供计算折线长度和多边形面积的公式
 */
...
const isPointOnPolyline = (point, polyline) => {
  ...
  for (let i = 0; i < pts.length - 1; i++) {
    ...
    if (point.lng >= Math.min(curPt.lng, nextPt.lng) && point.lng <= Math.max(curPt.lng, nextPt.lng) && point.lat >= Math.min(curPt.lat, nextPt.lat) && point.lat <= Math.max(curPt.lat, nextPt.lat)) {
      // 判断点是否在直线上公式
      let precision = (curPt.lng - point.lng) * (nextPt.lat - point.lat) - (nextPt.lng - point.lng) * (curPt.lat - point.lat);
...
const getPolylineDistance = (polyline) => {
  ...
  if (polyline instanceof BMap.Polyline || polyline instanceof Array) {
    ...
    for (let i = 0; i < pts.length - 1; i++) {
      let curPt = pts[i];
      let nextPt = pts[i + 1];
      let dis = getDistance(curPt, nextPt);
      totalDis += dis;
    }
    return totalDis;
  } else {
...
/**
 * 计算多边形面或点数组构建图形的中心点,注意：坐标类型只能是经纬度，且不适合计算自相交多边形的中心点
 * @param {Polygon|Array<Point>} polygon 多边形面对象或者点数组
 * @returns {Object} 多边形面或点数组构成图形的中心点
 */
const getPolygonCenter = (polygon) => {
  let path = [];
  if (polygon instanceof Array) {
    path = polygon;
  } else if(typeof(BMap) !== 'undefined' && polygon instanceof BMap.Polygon) {
    path = polygon.getPath(); // 返回多边型的点数组；ply是多边形覆盖物
  }
  let x = 0, y = 0;
