<template>
  <el-select
    ...
    <el-option
      ...
      :key="index"
      :label="$utilStr.getValueOfDeepObj(item, activeProps['label'])"
      :value="$utilStr.getValueOfDeepObj(item, activeProps['value'])">
      <span v-if="showCode" class="hdty-space-between">
        <span>{{ $utilStr.getValueOfDeepObj(item, activeProps['label']) }}</span>
        <span class="hdty-tips">{{ $utilStr.getValueOfDeepObj(item, activeProps['value']) }}</span>
      </span>
    </el-option>
  </el-select>
</template>
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      // 在改变会影响到绑定值的组件属性时（data/api/params/multiple/limit/visibleOption/removeOption），是否判定影响有效(true:影响绑定值，改变属性时清空绑定值；false:不影响绑定值，改变属性时不清空绑定值)
      checkInfluentially: {
        type: Boolean,
        default: false
      },
        ...
        if(!this.$utilStr.isEmpty(this.selectData)) {
          ...
          if(Array.isArray(this.currentValue)) {
            for (let i = 0;i < this.currentValue.length;i++) {
              let initialValue = this.$utilStr.generateDeepObj(this.activeProps['value'], this.currentValue[i]);
              let initialLabel = this.$utilStr.generateDeepObj(this.activeProps['label'], this.modelText[i] || this.currentValue[i]);
              let initialObj = this.$utilStr.deepObjMerge(initialValue, initialLabel);
              constData.push(initialObj);
            }
          } else if(!this.$utilStr.isEmpty(this.currentValue)) {
            let initialValue = this.$utilStr.generateDeepObj(this.activeProps['value'], this.currentValue);
            let initialLabel = this.$utilStr.generateDeepObj(this.activeProps['label'], this.modelText || this.currentValue);
            let initialObj = this.$utilStr.deepObjMerge(initialValue, initialLabel);
            constData.push(initialObj);
          }
          return constData;
    ...
    methods: {
        ...
        if(this.multiple) {
          if(Number(this.limit)) {
            ...
            if(!this.limitTipEl) {
              ...
              let tipText = document.createTextNode('最多选择['+this.limit+']项');
      ...
      handleAllButtonClick() {
        if(!this.allButtonEl.className.includes('is-active')) {
          ...
          this.$emit('currentChanged', this.allKeys, this.oldValue, this.allSelections);
        } else {
      ...
      handleChange(val) {
        ...
        this.activeData.forEach(item => {
          if(this.currentKeys.find(e => String(e) === String(this.$utilStr.getValueOfDeepObj(item, this.activeProps['value'])))) {
        ...
        if(this.multiple) {
          ...
          this.$emit('currentChanged', this.currentKeys, this.oldValue, currentSelections);
        } else {
          this.$nextTick(() => {
            this.$refs.selectRef.blur();
          });
          this.$emit('update:modelText', currentLabels[0] || '');
          this.$emit('currentChanged', this.currentKeys[0] || '', this.oldValue, currentSelections[0] || null);
        }
      },
      // select清空
              ...
              if(!this.$utilStr.isEmpty(this.value)) {
                if(Array.isArray(this.value)) {
                  ...
                  for (let e of this.value) {
                    let match = this.activeData.find(item => String(this.$utilStr.getValueOfDeepObj(item, this.activeProps['value'])) === String(e));
                    if(match) text.push(this.$utilStr.getValueOfDeepObj(match, this.activeProps['label']));
                  }
                } else {
                  let match = this.activeData.find(item => String(this.$utilStr.getValueOfDeepObj(item, this.activeProps['value'])) === String(this.value));
                  if(match) text = this.$utilStr.getValueOfDeepObj(match, this.activeProps['label']);
                }
              ...
              if(Array.isArray(text)) {
                this.$nextTick(async () => {
                  ...
                  if(text.length > 1) {
                    ...
                    let paddingSize = parseFloat(tagStyles.paddingLeft) + parseFloat(tagStyles.paddingRight);
                    let marginSize = parseFloat(tagStyles.marginLeft) + parseFloat(tagStyles.marginRight);
                    tagWidth += await this.$utilElem.getTextScreenWidth('+ ' + (text.length - 1), tagStyles.fontSize) + borderSize + paddingSize + marginSize;
                  }
