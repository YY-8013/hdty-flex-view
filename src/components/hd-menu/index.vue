...
<script>
  ...
  import { getTree, getTreeReverseDisplay } from '@/api/menu';
  export default {
    name: "index",
    provide() {
      return {
    ...
    methods: {
      ...
      loadList() {
        ...
        getTree(dataParams).then(response => {
          let { success, data } = response.data;
          if(success) {
            this.treeData = this.reconstructData(this.$utilStr.getValueOfDeepObj(data, this.defaultProps['response']));
      ...
      validateLimit(multipleData) {
        ...
        if(Number(this.limit) > 0
          ...
          try {
            let currentLimitMessageEls = document.querySelectorAll('.el-message.is-menu-select-limit');
