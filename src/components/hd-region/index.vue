<template>
  <div
    class="hdty-dialog-select hdty-region-select"
  ...
  >
    ...
    <el-dialog
      ...
      custom-class="hdty-dialog-select-panel hdty-region-select-dialog hdty-dialog-fit"
      append-to-body
...
import { getTree, getTreeReverseDisplay } from "@/api/region";
export default {
  name: "index",
  provide() {
    return {
  ...
  props: {
    ...
    // 在改变会影响到绑定值的组件属性时（multiple/limit/filterCode/showLevel/checkLevel/checkFinalLevel/checkStrictly），是否判定影响有效(true:影响绑定值，改变属性时清空绑定值；false:不影响绑定值，改变属性时不清空绑定值)
    checkInfluentially: {
      type: Boolean,
      default: false
    },
  ...
  methods: {
    ...
    loadRootNodeList(node) {
      ...
      let dataParams = {
        isFirstLoad: true,
        filterRegion: this.filterCode
      };

      // 判断是否过滤当前市区
      if (this.filterCity && (!this.filterCode || this.filterRegion)) {
        // 默认过滤仅展示当前市区的地区
        let currentRegion =
          this.$utilPublic.getUserRegionId().substring(0, 4) + "00";
        dataParams.filterRegion = currentRegion;
      }

      // 兼容下个版本弃用属性
      if (this.filterRegion && this.filterRegion !== dataParams.filterRegion) {
        dataParams.filterRegion = this.filterRegion;
      }
      ...
      if (Number(this.limit) > 0 && multipleData.length > this.limit) {
        try {
          let currentLimitMessageEls = document.querySelectorAll(
            ".el-message.is-region-select-limit"
          );
        ...
        this.$message({
          showClose: true,
          customClass: "is-fixed-top is-region-select-limit",
