...
#### 2.1.1 列配置表 (SYS_COLUMN_CONFIG)
存储数据统计列表的列配置信息,支持树形结构。

```sql
CREATE TABLE SYS_COLUMN_CONFIG (
    ID                VARCHAR2(32) PRIMARY KEY,           -- 主键ID
    PARENT_ID         VARCHAR2(32),                       -- 父节点ID (NULL表示根节点)
    LABEL             VARCHAR2(100) NOT NULL,             -- 列显示名称
    PROP              VARCHAR2(100),                      -- 数据字段名 (叶子节点必填)
    COLUMN_WIDTH      NUMBER(10),                         -- 列宽度
    ALIGN             VARCHAR2(20) DEFAULT 'center',      -- 对齐方式: left/center/right
    FIXED             VARCHAR2(20),                       -- 固定列: left/right/NULL
    SORTABLE          VARCHAR2(10) DEFAULT 'false',       -- 是否可排序: true/false
    DEFAULT_EXPAND    VARCHAR2(10) DEFAULT 'true',        -- 默认展开: true/false
    FORM_ID           VARCHAR2(32),                       -- 关联表单ID (可点击录入时配置)
    CALC_TYPE         VARCHAR2(50),                       -- 计算类型: sum/avg/count/custom
    CALC_RULE         CLOB,                               -- 计算规则 (JSON格式)

    
    -- 前端配置(新增,存储前端特有的渲染配置)
    COLUMN_CONFIG     CLOB,                               -- 列配置JSON(样式、格式化、条件显示等)
    
    SORT_NUM          NUMBER(10) DEFAULT 0,               -- 排序号
    ...
    CREATE_USERNAME   VARCHAR2(50),                       -- 采集人姓名
    CREATE_TIME       VARCHAR2(14),                       -- 采集时间
    CREATE_ORG_ID     VARCHAR2(12),                       -- 采集人机构
    UPDATE_USER_GMSFHM VARCHAR2(18),                      -- 操作人身份证号
    UPDATE_USERNAME   VARCHAR2(50),                       -- 操作人姓名
    UPDATE_TIME       VARCHAR2(14),                       -- 操作时间
    UPDATE_ORG_ID     VARCHAR2(12),                       -- 操作人机构
    MEMO              VARCHAR2(500),                      -- 备注
    
    CONSTRAINT FK_COLUMN_PARENT FOREIGN KEY (PARENT_ID) REFERENCES SYS_COLUMN_CONFIG(ID)
);

-- 索引
CREATE INDEX IDX_COLUMN_PARENT ON SYS_COLUMN_CONFIG(PARENT_ID);
CREATE INDEX IDX_COLUMN_ZXBS ON SYS_COLUMN_CONFIG(ZXBS);
CREATE INDEX IDX_COLUMN_SORT ON SYS_COLUMN_CONFIG(SORT_NUM);

-- 注释
COMMENT ON TABLE SYS_COLUMN_CONFIG IS '统计列配置表';
COMMENT ON COLUMN SYS_COLUMN_CONFIG.ID IS '主键ID';
COMMENT ON COLUMN SYS_COLUMN_CONFIG.PARENT_ID IS '父节点ID';
COMMENT ON COLUMN SYS_COLUMN_CONFIG.LABEL IS '列显示名称';
COMMENT ON COLUMN SYS_COLUMN_CONFIG.PROP IS '数据字段名';
COMMENT ON COLUMN SYS_COLUMN_CONFIG.FORM_ID IS '关联表单ID(可点击录入)';
COMMENT ON COLUMN SYS_COLUMN_CONFIG.DEFAULT_EXPAND IS '默认展开状态';
COMMENT ON COLUMN SYS_COLUMN_CONFIG.COLUMN_CONFIG IS '列前端配置JSON(样式、格式化、条件显示等)';
COMMENT ON COLUMN SYS_COLUMN_CONFIG.STATUS IS '启用状态:1-启用 0-停用';
COMMENT ON COLUMN SYS_COLUMN_CONFIG.STATUS_REMARK IS '启用/停用备注说明';
COMMENT ON COLUMN SYS_COLUMN_CONFIG.CREATE_USER_GMSFHM IS '采集人身份证号码';
COMMENT ON COLUMN SYS_COLUMN_CONFIG.ZXR_USER_GMSFHM IS '注销人身份证号码';
```
...
```sql
-- 实有人口父节点
INSERT INTO SYS_COLUMN_CONFIG (ID, PARENT_ID, LABEL, PROP, SORT_NUM,
    ZXBS, CREATE_USER_ID, CREATE_USERNAME, CREATE_TIME, CREATE_ORG_ID,
...
-- 总数子节点(不可点击)
INSERT INTO SYS_COLUMN_CONFIG (ID, PARENT_ID, LABEL, PROP, COLUMN_WIDTH, SORT_NUM, COLUMN_CONFIG,
    ZXBS, CREATE_USER_ID, CREATE_USERNAME, CREATE_TIME, CREATE_ORG_ID,
    UPDATE_USER_ID, UPDATE_USERNAME, UPDATE_TIME, UPDATE_ORG_ID)
...
-- 抽查总数子节点(可点击,有样式)
INSERT INTO SYS_COLUMN_CONFIG (ID, PARENT_ID, LABEL, PROP, COLUMN_WIDTH, FORM_ID, SORT_NUM, COLUMN_CONFIG,
    ZXBS, CREATE_USER_ID, CREATE_USERNAME, CREATE_TIME, CREATE_ORG_ID,
    UPDATE_USER_ID, UPDATE_USERNAME, UPDATE_TIME, UPDATE_ORG_ID)
...
#### 2.1.2 表单配置表 (SYS_FORM_CONFIG)
存储动态表单的基础信息,支持分表配置。

```sql
CREATE TABLE SYS_FORM_CONFIG (
    ID                VARCHAR2(32) PRIMARY KEY,           -- 主键ID
    FORM_CODE         VARCHAR2(100) NOT NULL UNIQUE,      -- 表单编码
    FORM_NAME         VARCHAR2(200) NOT NULL,             -- 表单名称
    DATA_TABLE        VARCHAR2(100) DEFAULT 'BIZ_DATA_COMMON',  -- 数据存储表名(默认为BIZ_DATA_COMMON)
    FORM_TYPE         VARCHAR2(50) DEFAULT 'COMMON',      -- 表单类型: COMMON-通用/CUSTOM-自定义
    DESCRIPTION       VARCHAR2(500),                      -- 表单说明
    ...
    MEMO              VARCHAR2(500),                      -- 备注
    VERSION           NUMBER(10) DEFAULT 1                -- 版本号
);

-- 索引
CREATE INDEX IDX_FORM_CODE ON SYS_FORM_CONFIG(FORM_CODE);
CREATE INDEX IDX_FORM_ZXBS ON SYS_FORM_CONFIG(ZXBS);
CREATE INDEX IDX_FORM_TABLE ON SYS_FORM_CONFIG(DATA_TABLE);

-- 注释
COMMENT ON TABLE SYS_FORM_CONFIG IS '表单配置表';
COMMENT ON COLUMN SYS_FORM_CONFIG.FORM_CODE IS '表单编码(唯一)';
COMMENT ON COLUMN SYS_FORM_CONFIG.DATA_TABLE IS '数据存储表名(默认BIZ_DATA_COMMON,支持分表)';
COMMENT ON COLUMN SYS_FORM_CONFIG.VERSION IS '表单版本号';
```

**示例数据**:
```sql
-- 抽查记录表单(使用默认表)
INSERT INTO SYS_FORM_CONFIG (ID, FORM_CODE, FORM_NAME, DATA_TABLE, DESCRIPTION,
    ZXBS, CREATE_USER_ID, CREATE_USERNAME, CREATE_TIME, CREATE_ORG_ID,
    UPDATE_USER_ID, UPDATE_USERNAME, UPDATE_TIME, UPDATE_ORG_ID) 
