...
│  │  │     ├─ DateField.vue
│  │  │     └─ TextareaField.vue
│  │  ├─ MultiLevelTable/           # 多级表头表格
│  │  │  ├─ index.vue
│  │  │  └─ ColumnRender.vue        # 列渲染器
│  │  ├─ TreeSelect/                # 树形选择器
│  │  │  └─ index.vue
│  │  └─ ColumnConfig/              # 列配置组件
│  │     ├─ index.vue
│  │     ├─ TreeEditor.vue          # 树形编辑器
│  │     └─ ColumnEditor.vue        # 列编辑器
│  ├─ views/                        # 页面
│  │  ├─ stat/                      # 数据统计模块
│  │  │  ├─ DataStat.vue            # 数据统计列表页
│  │  │  └─ DataDetail.vue          # 明细数据弹窗
│  │  ├─ config/                    # 配置管理模块
│  │  │  ├─ ColumnConfig.vue        # 列配置管理
│  │  │  ├─ FormConfig.vue          # 表单配置管理
│  │  │  └─ FormFieldConfig.vue     # 表单字段配置
│  │  └─ dashboard/                 # 仪表盘
│  │     └─ index.vue
│  ├─ router/                       # 路由配置
│  │  └─ index.js
...
#### 2.1.1 组件设计: MultiLevelTable

**功能**: 根据树形列配置动态生成多级表头

**核心思路**:
- 递归渲染`<el-table-column>`
- 支持展开/收起控制
- 单元格点击事件处理
...
#### 2.2.1 组件设计: DynamicForm

**功能**: 根据表单配置动态渲染表单

**核心思路**:
- 根据 ITEM_CONFIG JSON 配置渲染组件
- 根据 ITEM_TYPE 选择不同组件
- 支持表单验证(从 JSON 中读取 rules)
- 数据双向绑定
...
export default {
  ...
  props: {
    ...
    formItems: {  // 组件列表(从 SYS_FORM_ITEM_CONFIG 查询)
      type: Array,
      default: () => []
    },
    initData: {
  ...
  methods: {
    ...
    initFormRules() {
      ...
      this.formItems.forEach(item => {
        ...
        // 从 ITEM_CONFIG 的 rules 字段读取验证规则
        if (config.rules) {
          // 必填验证
...
export default {
  ...
  props: {
    item: {  // 完整的 item 对象(包含 itemConfig JSON)
      type: Object,
      required: true
    },
    value: {
...
export default {
  ...
  props: {
    item: Object,
    config: Object,  // 解析后的 ITEM_CONFIG
    value: [String, Number]
  },
  computed: {
    inputType() {
...
export default {
  ...
  computed: {
    // 从 ITEM_CONFIG 的 options.data 获取选项
    options() {
        ...
        <el-card>
          ...
          >
            ...
            <el-form-item label="数据字段" prop="prop">
              <el-input v-model="currentColumn.prop" placeholder="叶子节点必填" />
            </el-form-item>
            ...
            <el-form-item>
              ...
              <el-button @click="currentColumn = null">取消</el-button>
            </el-form-item>
          </el-form>
          
          <el-empty v-else description="请选择要编辑的列" />
        </el-card>
      </el-col>
    </el-row>
  </div>
...
export default {
  ...
  methods: {
    ...
    async handleDelete(data) {
      try {
        await this.$confirm('确定删除该列?', '提示', { type: 'warning' })
  ...
  <div class="form-config">
    ...
    <el-table :data="formList" border>
      ...
      <el-table-column label="操作" width="300">
        <template slot-scope="scope">
          <el-button size="mini" @click="handleEditForm(scope.row)">编辑</el-button>
...
// 获取表单组件(以前是 getFormFields,现在改为 getFormItems)
export function getFormItems(formId, useType) {
  return request({
    url: `/api/form/${formId}/items`,
    method: 'get',
    params: { useType }  // 01-app表单 02-app列表 03-pc列表 04-pc表单
  })
}
...
describe('DynamicForm.vue', () => {
  it('renders form fields correctly', () => {
    const formFields = [
      { id: '1', fieldName: 'name', fieldLabel: '姓名', fieldType: 'input' }
    ]
