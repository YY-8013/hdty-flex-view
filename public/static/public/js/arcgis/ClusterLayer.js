define([
  ...
  return declare([GraphicsLayer], {
    ...
    onClick: function(e) {
      ...
      for (var i = 0, len = this._clusterData.length; i < len; i++) {
        // 匹配点击的marker及它包含的子类
        if (e.graphic.attributes.clusterId == this._clusterData[i].attributes.clusterId) {
          singles.push(this._clusterData[i]);
        }
      }
      if(singles.length > 1){ // 如果点击的marker包含数量大于1，进行放大展示
        var currentScale = this._map.getZoom(); // 当前地图缩放比例
        var expandZoom = currentScale + 2;  // 增大缩放比例         
        if(expandZoom > 0){
          this._map.setZoom(expandZoom);     // 放大
        }else{
          this._map.setScale(50000);           // 缩放比例 （比例尺）                       
        }
        this._map.centerAndZoom(e.mapPoint);  // 定位      
      }else{  // 如果点击的maker包含数量为1
        e.stopPropagation();  // 停止事件的传播，这样弹出窗口才不会出现闪现的情况            
    ...
    _clusterCreate: function(p) {
      ...
      var cluster = {
        "x": p.x,
        "y": p.y,
        "singleMarkerSymbol": p.markerSymbol,
        "singleTextSymbol": p.textSymbol,
        "attributes" : {
          "clusterCount": 1,
          "clusterId": clusterId,
    ...
    _showCluster: function(c) {
      ...
      var clusterCount = c.attributes.clusterCount;
      this.add(
        new Graphic(
          point,
          clusterCount == 1 ? c.singleMarkerSymbol : null,  // 如果只有一个点显示自定义标记
          c.attributes
        )
      );
